...
class curl
{
    ...
    public function execute()
    {
        ...
        if ($curl_errno !== CURLE_OK) {
            ...
            if(strpos($curl_error, 'bind') !== false) {
                ...
                $result = $this->execute();
            } elseif(array_key_exists($curl_errno, $this->onErrors)) {
    ...
    public function getCharsetFromHeader()
    {
        ...
        $result = null;
        if ($this->curl !== null && is_resource($this->curl) && preg_match($pattern, curl_getinfo($this->curl)['content_type'], $match)) {
    ...
    {
        return $this->options[CURLOPT_CONNECTTIMEOUT] = (int) $seconds;
    }
    ...
    {
        return $this->options[CURLOPT_TIMEOUT] = (int) $seconds;
    }
